...
# One "travels cleanly" (green) and one "needs work" (yellow) per source language,
# so the rail itself shows the score discriminating. No red band: with Sarvam,
# content rarely collapses to a near-rewrite, so "needs work" (yellow) is the
# honest low end. Label format: "Language · Sentence-case description".
SAMPLES = [
    ...
    ("English · Skincare tips", "https://youtube.com/shorts/obRs6VrF9FE", "yellow"),
    ("Hindi · Street-food vlog", "https://youtube.com/shorts/L5mWKH07TBQ", "yellow"),
    ("Tamil · Shirt seller", "https://youtube.com/shorts/mRFs19QPAwI", "yellow"),
]

# Two-band signal for the sample grid: green = travels cleanly, yellow = needs work.
SAMPLE_BANDS = [
    ("green", "Travels cleanly", "#2f8f5f"),
    ("yellow", "Needs work", "#c98a2b"),
]
...
def main():
    ...
    with st.sidebar:
        ...
        st.caption(f"Clips are trimmed to the first "
                   ...
                   f"free Sarvam tier.")
        st.markdown("---")
        st.markdown("**Try an example**\n\nCopy one, paste it above. The colour is how "
                    "well it travels.")
        blocks = ""
